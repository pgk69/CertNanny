...
#  if (!CertNanny::Util->mangle(VALUE => $entry->{TrustedRootCA}->{GENERATED}->{Directory}, MANGLE => 'FILE')) {
...
#  }
  ...
  if (!CertNanny::Util->mangle(VALUE => $entry->{TrustedRootCA}->{GENERATED}->{Location}, MANGLE => 'FILE')) {
...
sub getCertLocation {
  ...
  if ($args{CAChain}) {
    foreach ('Directory', 'File') {
      if (my $location = CertNanny::Util->mangle(VALUE => $entry->{CAChain}->{GENERATED}->{Directory}, MANGLE => 'FILE')) {
        $rc->{lc($_)} = $location;
      }
    }
  }
...
sub getInstalledCAs {
  ...
  if (!defined($args{TARGET}) or ($args{TARGET} eq 'LOCATION')) {
    if (defined(my $locName = CertNanny::Util->mangle(VALUE => $entry->{location}, MANGLE => 'FILE'))) {
    ...
    if (!defined($availableRootCAs)) {
      ...
      if (!$rc) {
        ...
        if (!File::Copy::copy($locName, $entry->{location})) {
          ...
          CertNanny::Util->wipe(FILE => $locName, SECURE => 1);
        }
      }
    }
  }
